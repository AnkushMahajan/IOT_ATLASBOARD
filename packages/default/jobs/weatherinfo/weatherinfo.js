...
module.exports = {
  ...
  onRun: function (config, dependencies, jobCallback) {
    ...
    /*
     ...
     It is a good idea to cover this with unit tests as well (see test/weatherinfo file)
     ...
     Have a look at test/weatherinfo for an example of how to unit tests this easily by mocking easyRequest calls

     */

    dependencies.easyRequest.JSON('http://api.openweathermap.org/data/2.5/forecast?appid=' + config.APIKey +'&q='+ config.city + ',' + config.countryCode, function (err, hourlyRes) {
      dependencies.easyRequest.JSON('http://api.openweathermap.org/data/2.5/forecast/daily?appid=' + config.APIKey +'&q='+ config.city + ',' + config.countryCode, function (err, dailyRes) {
