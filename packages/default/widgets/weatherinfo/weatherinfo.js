widget = {
  ...
  onData: function(el, data) {
    ...
    if (data.hourlyContent && data.hourlyContent.list && data.hourlyContent.list.length > 0) {
      // create table
      var div = $('<div style="height:100px;overflow:auto;"></div>').appendTo(mainContent);
    ...
    if(data.dailyContent && data.dailyContent.list && data.dailyContent.list.length > 0) {
      ...
      data.dailyContent.list.forEach(function(weatherItem) {
        ...
        var td3 = $('<td class="maxTemp">' + Math.round(parseInt(weatherItem.temp.max - 273.15), 10) + '&deg;</td>').appendTo(tr);
        var td3 = $('<td class="minTemp">' +Math.round(parseInt( weatherItem.temp.min - 273.15), 10) + '&deg;</td>').appendTo(tr);
      });
    }
  }
};